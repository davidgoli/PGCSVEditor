...
static NSString *priorityCSVPath = nil;
static NSString *priorityCSVStagingPath = nil;
static NSString* prepPriorityCSVPath(NSString *val, NSString *where) {
    if(val == nil) {
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES);
        NSString *appSupportPath = [paths objectAtIndex:0];
        val = [NSString stringWithFormat:@"%@/%@/", appSupportPath, where];
        [val retain];
    }
    return val;
}
NSString *getPriorityCSVPath() {
    priorityCSVPath = prepPriorityCSVPath(priorityCSVPath, @"priority");
    return priorityCSVPath;
}
NSString *getPriorityCSVStagingPath() {
    priorityCSVStagingPath = prepPriorityCSVPath(priorityCSVStagingPath, @"priority-staging");
    return priorityCSVStagingPath;
}

-(BOOL)openFile:(NSString*)fileName {
    // if the requested CSV file has a version in the high priority folder, then
    // use that version instead
    NSString *priorityFileName = [getPriorityCSVPath() stringByAppendingString:[fileName lastPathComponent]];
    if([[NSFileManager defaultManager] fileExistsAtPath:priorityFileName])
...
- (id)initWithFilepath:(NSString *)filepath {
	...
	if (self != nil) {
		...
		if (open) {
			...
			[content removeObjectAtIndex:0]; //remove header
			[self setup:content];
			[self closeFile];
		} else {
